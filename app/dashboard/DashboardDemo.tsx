...
const PIE_COLORS = ["#82d173", "#5fae52", "#a8e39c", "#4a8a3f", "#c5edbc", "#3d7534", "#9bd98f", "#2e5a27", "#6bc25c", "#d4f0cc"];
...
export default function DashboardDemo({
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto bg-brand-bg text-brand-text">
      <header className="flex items-center justify-between border-b border-brand-border px-6 py-4">
        <div>
          <h1 className="text-lg font-semibold">Dashboard</h1>
          <p className="text-xs text-brand-muted-2">Demo · current finances</p>
        </div>
        <button
          onClick={() => setShowFilters((s) => !s)}
          className="flex items-center gap-2 rounded-xl border border-brand-border bg-brand-surface px-4 py-2 text-sm text-brand-muted hover:border-brand-border"
        >
      ...
      {/* Filter bar */}
      {showFilters && (
        <div className="flex flex-wrap items-center gap-3 border-b border-brand-border bg-brand-surface/60 px-6 py-4">
          <div className="flex gap-1 rounded-xl border border-brand-border bg-brand-surface p-1">
            {PERIODS.map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${period === p ? "bg-brand-accent text-black" : "text-brand-muted hover:text-brand-text"}`}
              >
                {p}
              </button>
            ))}
          ...
          <div className="flex gap-1 rounded-xl border border-brand-border bg-brand-surface p-1">
            {(["all", "expense", "income"] as const).map((v) => (
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-medium capitalize transition-colors ${flowType === v ? "bg-brand-accent text-black" : "text-brand-muted hover:text-brand-text"}`}
              >
                {v}
              </button>
            ))}
          ...
          <select
            ...
            className="rounded-xl border border-brand-border bg-brand-surface px-3 py-2 text-xs text-brand-muted outline-none"
          >
          ...
          <input
            ...
            className="rounded-xl border border-brand-border bg-brand-surface px-3 py-2 text-xs text-brand-muted outline-none placeholder:text-brand-muted-3"
          />

          {(category !== "all" || paymentMethod || flowType !== "all") && (
            <button
              onClick={() => { setCategory("all"); setPaymentMethod(""); setFlowType("all"); }}
              className="flex items-center gap-1 text-xs text-brand-muted-2 hover:text-brand-muted"
            >
              <X size={12} /> Clear
            </button>
          )}
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-6 py-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-brand-accent/30 bg-linear-to-br from-brand-accent/10 to-transparent p-5">
            <div className="mb-2 flex items-center gap-2 text-brand-accent"><Wallet size={16} /><span className="text-xs font-semibold uppercase tracking-wide">Net Balance</span></div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border border-brand-border bg-brand-surface p-5">
            <h3 className="mb-4 text-sm font-semibold text-brand-muted">Spending by category</h3>
            {stats.categoryData.length === 0 ? (
              <p className="py-10 text-center text-xs text-brand-muted-3">No expense data for this filter</p>
            ) : (
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  ...
                  <Tooltip formatter={(v) => fmt(typeof v === "number" ? v : Number(v ?? 0))} contentStyle={{ background: "#171717", border: "1px solid #262626", borderRadius: 8, fontSize: 12 }} />
                </PieChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-brand-border bg-brand-surface p-5">
            <h3 className="mb-4 text-sm font-semibold text-brand-muted">Trend</h3>
            {trendData.length === 0 ? (
              <p className="py-10 text-center text-xs text-brand-muted-3">No data for this filter</p>
            ) : (
              <ResponsiveContainer width="100%" height={240}>
        ...
        <div className="rounded-2xl border border-brand-border bg-brand-surface">
          <div className="border-b border-brand-border px-5 py-4">
            <h3 className="text-sm font-semibold text-brand-muted">Transactions ({filtered.length})</h3>
          </div>
          <div className="divide-y divide-brand-border">
            {filtered.length === 0 && <p className="px-5 py-10 text-center text-xs text-brand-muted-3">No transactions match this filter</p>}
              ...
              return (
                <div key={item.id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    ...
                    <div className="text-xs text-brand-muted-2">{item.category || "General"} · {new Date(item.receipt_date).toLocaleDateString()}</div>
                  </div>
                  <div className={`text-sm font-semibold ${isIncome ? "text-brand-accent" : "text-brand-text"}`}>
